...
class Mod_slider extends CI_Model
{
	...
	function save_slider_data()
	{
		...
		if ( $_FILES['image']['error'] == 0 and $_FILES['image']['size'] > 0 ) {
			...
			$image = $this->mod_file_upload->upload_file( 'sliders', 'image', ['width' => 2000, 'height' => 2000, 'size' => 1024] );
	...
	function update_slider_data( $id )
	{
		...
		if ( $_FILES['image']['error'] == 0 and $_FILES['image']['size'] > 0 ) {
			...
			if ( $image['status'] ) {
				...
				$slider = $this->Mod_common->get_row_data_by_id( 'oe_slider', $id );
				unlink( $slider->image );
		...
		$this->db->update( 'oe_slider', $data, ['id' => $id] );
	...
	function get_records( $limit = 20, $row = 0 )
	{
		...
		$query = $this->db->get( 'oe_slider', $limit, $row );
		if ( $query->num_rows() > 0 ) {
