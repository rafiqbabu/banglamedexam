				...
				<div class="col-md-9" id="i">
					<div class="widget profile-widget">
						...
						<h4 class="widget-title"><?php echo $page_title; ?><span></span></h4>
							...
							<?php
                                //check institute, course, faculty & subject for sif users only
                                ...
                                if (!empty($query->num_rows())) {
                                    foreach ($query->result() as $rowi) {
                                        // redirect(site_url("user/$id/exam-selection-sif")."?i=$rowi->institute_id&c=$rowi->course_id&f=$rowi->faculty_id&s=$rowi->subject_id#s);
                                    }
                                }
													...
													<?php
														...
														if ($query->num_rows() > 0) {
															foreach ($query->result() as $row2) {
																echo "<li style='background-color:#8e44ad;'><a href='".site_url("user/$user->id/exam-src-faculty-sif")."?i={$row->id}&c={$row2->id}#f'>{$row2->course_name}</a></li>";
															}
														}
													?>
												</ul>
