				...
				<div class="col-md-9" id="c">
					<div class="widget profile-widget">
						...
						<?php 
						    if ($_GET['i'] == 4){
						?>
    						<form action="<?php echo site_url("user/$user->id/exam-selection"); ?>#s" method="GET">
    							...
    							<input type="hidden" name="s" value="<?php echo $rows->id; ?>">
    							<input type="hidden" name="candidate" value="<?php echo 0; ?>">
    							<?php echo $i.$c.$f.$s; ?>
						...
						?>
					        ...
					        <form action="<?php echo site_url("user/$user->id/exam-selection"); ?>#s" method="GET">
    							...
    							<a style="background-color:#d35400; color:#FFFFFF; padding:10px; border-radius:10px;" href="<?php echo site_url("user/$user->id/exam-selection")."?i={$rowi->id}&c={$rowc->id}&f={$rowf->id}&s={$rows->id}&candidate=2#s"; ?>">Private</a>&nbsp;&nbsp;&nbsp;
    							<br><br>
    							<input type="radio" name="candidate" value="3" required>
    							<a style="background-color:#800080; color:#FFFFFF; padding:10px; border-radius:10px;" href="<?php echo site_url("user/$user->id/exam-selection")."?i={$rowi->id}&c={$rowc->id}&f={$rowf->id}&s={$rows->id}&candidate=3#s"; ?>">BSMMU</a>	
    							<br><br>
    							<input type="radio" name="candidate" value="1" required>	
    							<a style="background-color:#006400; color:#FFFFFF; padding:10px; border-radius:10px;" href="<?php echo site_url("user/$user->id/exam-selection")."?i={$rowi->id}&c={$rowc->id}&f={$rowf->id}&s={$rows->id}&candidate=1#s"; ?>">Government</a>&nbsp;&nbsp;&nbsp;
