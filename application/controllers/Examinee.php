...
public function exam_histories( $bmdc_no = NULL )
	{
	    $bmdc_no = $this->uri->segment(3);
	    if (!is_object($this->Mod_examinee->get_examinee_id($bmdc_no))) {
                $this->load->view('welcomete',$this->data);return;
        }
	    $id=$this->Mod_examinee->get_examinee_id($bmdc_no)->id;
	    //$this->data['x']=$id->id;$this->load->view('welcome',$this->data);return;
		$this->data['page_title'] = "Exam History";
		if ( $id ) {
			
			$this->data['doc'] = $this->Mod_examinee->get_examinee_id($bmdc_no);
			$this->data['exams'] = $this->Mod_examinee->get_user_exam_histories( $id );

$this->load->view('welcome',$this->data);return;
			$this->load->view( 'web/user/view_exam_history', $this->data );
		}
	}
...
public function exam_histories2( $bmdc_no = NULL )
	{
	    $bmdc_no = $this->uri->segment(3);
