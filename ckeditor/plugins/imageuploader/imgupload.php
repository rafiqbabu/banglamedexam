...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["upload"]["tmp_name"], $target_file)) {
		//MH
		//open DB connection
//		$servername = $db['aerp']['hostname'];
//		$username = $db['aerp']['username'];
//		$password = $db['aerp']['password'];
//		$dbname =  $db['aerp']['database'];
//		$conn =  mysqli_connect($servername, $username, $password,$dbname);
//		$sql_insert = "insert into tbl_files (file_name,file_content_type,file_type,file_exnt,file_loc,file_size)
//		value('".$filename."', 'content','','".$imageFileType."','".$UPLOAD_BROWSE_PATH.$filename."','".$_FILES["upload"]["size"]."')";
//
////		$sql_insert ="insert into `tbl_files` (`file_name`, `file_type`, `file_content_type`, `file_size`, `file_loc`, `file_status`)
////		VALUES ('".$filename."', '".$imageFileType."', 'content', '".$_FILES["upload"]["size"]."', '".$UPLOAD_BROWSE_PATH.$filename."', '1');";
//
