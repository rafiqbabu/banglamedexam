...
<table border="0" align="center" cellpadding="5" cellspacing="0" width="1000">
    <tr align="left">
        <td>
            ...
            <?php 
                $qpn = mysqli_query($link, "SELECT * FROM oe_package WHERE id = '$package_id'"); 
                while($dpn = mysqli_fetch_array($qpn)){
                    echo "Package Name : ".$dpn[name];
                } 
            ?>
            </font>
        </td>
...
<?php
	...
	if ($package_id != '0') {
	    $sql="SELECT * FROM oe_doc_exams WHERE status = '1' AND doc_id = '$id' AND package_id = '$package_id' ORDER BY id DESC";
	}
	else {
	    $sql="SELECT * FROM oe_doc_exams WHERE status = '1' AND doc_id = '$id' AND package_id IS NULL ORDER BY id DESC";
	}
    ...
    <tr align="center" bgcolor="#ececec">
        ...
        <td>
            <?php
                //Overall Position
                $sp=1;
                $q6 = mysqli_query($link, "SELECT * FROM oe_doc_exams WHERE status = '1' AND exam_id = '$exam_id' ORDER BY final_mark DESC");
